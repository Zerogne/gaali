...
function normalizeLogForClient(doc: any): TruckLog {
  ...
  return {
    ...
    truckWeight: resolvedTruckWeight ?? doc.truckWeight,
...
export async function saveTruckLog(
  ...
  try {
    ...
    const { truckWeight: tw, carWeight: cw, trailerWeight: trw } = log as any
    const truckWeight = tw ?? cw
    const trailerWeight = trw
    const cleanedLog = {
      ...log,
      ...
      trailerWeight: trw != null && trw > 0 ? trw : undefined,
