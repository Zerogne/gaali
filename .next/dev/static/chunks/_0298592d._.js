...
function DriversPage() {
    ...
    const handleAddDriver = async ()=>{
        ...
        if (editingDriver) {
            ...
            // Check for similar/duplicate driver names (excluding current driver)
            const existingNames = drivers.filter((d)=>d.id !== editingDriver).map((d)=>d.name);
            const similarDriver = findSimilarValue(driverData.name.trim(), existingNames);
            if (similarDriver) {
                setDuplicateValue(similarDriver);
        ...
        // Check for similar/duplicate driver names
        const existingNames = drivers.map((d)=>d.name);
        const similarDriver = findSimilarValue(driverData.name.trim(), existingNames);
        if (similarDriver) {
            setDuplicateValue(similarDriver);
            setDuplicateDialogOpen(true);
...
//# sourceMappingURL=_0298592d._.js.map