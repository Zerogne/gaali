...
export function TruckInSection({ onSave }: TruckInSectionProps) {
  ...
  const [netWeight, setGrossWeight] = useState("")
  ...
  const validateForm = (): boolean => {
    ...
    if (!netWeight.trim()) newErrors.weight = "Gross weight is required"
    else if (isNaN(Number(netWeight)) || Number(netWeight) <= 0) newErrors.weight = "Weight must be a positive number"
    setErrors(newErrors)
  ...
  return (
    <Card className="shadow-md hover:shadow-lg transition-shadow duration-200 border-border/50 bg-gradient-to-br from-card to-card/98">
      <CardHeader className="pb-4 border-b border-border/50">
        <CardTitle className="text-xl font-bold tracking-tight">Truck OUT – Gate Exit</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6 pt-6">
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            <Label htmlFor="driver-name-out" className="text-sm font-medium">Driver Name</Label>
            <Input id="driver-name-out" value={driverName} onChange={(e) => setDriverName(e.target.value)} placeholder="Enter driver name" aria-invalid={!!errors.driverName} className="transition-all focus:border-primary/60" />
            {errors.driverName && <p className="text-xs text-destructive font-medium mt-1">{errors.driverName}</p>}
          </div>
          <div className="space-y-2">
            <Label htmlFor="cargo-type-out" className="text-sm font-medium">Cargo Type</Label>
            <Select value={cargoType} onValueChange={setCargoType}>
              <SelectTrigger id="cargo-type-out" aria-invalid={!!errors.cargoType} className="transition-all focus:border-primary/60">
          ...
          <div className="space-y-2">
            <Label htmlFor="net-weight-out" className="text-sm font-medium">Net Weight / Remaining Cargo (kg)</Label>
            <Input id="net-weight-out" type="number" value={netWeight} onChange={(e) => setGrossWeight(e.target.value)} placeholder="Enter gross weight" aria-invalid={!!errors.weight} className="transition-all focus:border-primary/60" />
